...
def extract_features(audio, model, tokenizer, device):
    inputs = tokenizer(audio, return_tensors="pt", sampling_rate=16000, padding="longest").to(device)
    with torch.no_grad():
...
def predict(audio):
    ...
    feature_vector = extract_features(audio, wav2vec_model, tokenizer, device)
